...
let heightMap, shadowMap, blurTempMap; 
let lut = new Uint8Array(256 * 3); // Flattened 1D color LUT for superior cache locality

// Generates the 1D LUT array to eliminate array-instantiation inside render loops
...
// O(N) Sliding window box blur: massively out-performs brute-force nested looping
...
function generateTerrain() {
    ...
    for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
            ...
            heightMap[x + y * cols] = val < 0 ? 0 : (val > 1 ? 1 : val);
        }
    }
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                ...
                for (let step = 1; step <= maxSteps; step++) {
                    ...
                    let rx = rayX | 0, ry = rayY | 0; // Fast Float-to-Int cast
        ...
        for (let x = 0; x < cols; x++) {
            ...
            if (doShading) {
                if (h > waterLvl) {
                    ...
                    let waterShadow = 1.0 + (shadowMap[i] - 1.0) * 0.6; // Lerp implementation inline
                    r *= waterShadow; g *= waterShadow; b *= waterShadow;
                }
            }

            let pIdx = i << 2; // Fast multiply by 4
